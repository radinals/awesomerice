...
return {
  ...
  },
  {
    ...
    bindings = {
      ...
      {mod = { M.MOD }, key = "t",                 press = function(c) c.ontop = not c.ontop end, desc = "Toggle keep on top"},
      {mod = { M.MOD }, key = "n",                 press = function(c) c.minimized = true end, desc = "Minimize"},
      {mod = { M.MOD }, key = "m",                 press = function(c) c.maximized = not c.maximized; c:raise() end, desc = "(Un)maximize"},
      {mod = { M.MOD, M.CONTROL }, key = "m",      press = function(c) c.maximized_vertical = not c.maximized_vertical; c:raise() end, desc = "(Un)maximize vertically"},
      {mod = { M.MOD, M.SHIFT }, key = "m",        press = function(c) c.maximized_horizontal = not c.maximized_horizontal; c:raise() end, desc = "(Un)maximize horizontally"},
    },
  },
}
